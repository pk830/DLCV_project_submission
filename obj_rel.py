...
def process_novel_all_split(train_file, test_file):
    # 1. Split Both Vocabularies (Completely Disjoint Sets)
    ...
    print(f"Filtering based on Novel Objects + Novel Relationships Split...")
    print(f"Train Vocab -> Objects: {len(train_objects)}, Relationships: {len(train_relationships)}")
    print(f"Test Vocab  -> Objects: {len(test_objects)}, Relationships: {len(test_relationships)}")
    ...
    for entry in pooled_data:
        ...
        # Filter for train (train objects + train relationships)
        train_filtered = filter_entry(entry_for_train, train_objects, train_relationships)
        if train_filtered:
            train_split.append(train_filtered)

        # Filter for test (test objects + test relationships)
        test_filtered = filter_entry(entry_for_test, test_objects, test_relationships)
        if test_filtered:
            test_split.append(test_filtered)

    # Save the results
    print("\nSaving 'novel_all_train.json' and 'novel_all_test.json'...")
    with open('novel_all_train.json', 'w', encoding='utf-8') as f:
        json.dump(train_split, f, indent=2)
    with open('novel_all_test.json', 'w', encoding='utf-8') as f:
        json.dump(test_split, f, indent=2)
        
    print(f"Done! Train set size: {len(train_split)}. Test set size: {len(test_split)}.")

if __name__ == "__main__":
    process_novel_all_split("../gqa_train.json", "../gqa_test.json")