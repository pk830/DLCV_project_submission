...
def process_novel_objects_split(train_file, test_file):
    ...
    test_objects = {"ocean", "car", "picture", "hand", "snow", "horn", "woman", "sweater", "container",
    ...
    print(f"Test Objects ({len(test_objects)}): {test_objects}")

    for entry in pooled_data:
        # We need deep copies so filtering the train set doesn't ruin the data for the test set
        entry_for_train = copy.deepcopy(entry)
        entry_for_test = copy.deepcopy(entry)

        # Filter for train (only keeps train objects)
        train_filtered = filter_entry(entry_for_train, train_objects, shared_relationships)
        if train_filtered:
            train_split.append(train_filtered)

        # Filter for test (only keeps test objects)
        test_filtered = filter_entry(entry_for_test, test_objects, shared_relationships)
        if test_filtered:
            test_split.append(test_filtered)

    # Save the results
    print("\nSaving 'novel_objects_train.json' and 'novel_objects_test.json'...")
...
if __name__ == "__main__":
    process_novel_objects_split("../gqa_train.json", "../gqa_test.json")